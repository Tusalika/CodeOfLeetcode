...
public class B_1025 {
    ...
    public static void main(String[] args) throws IOException {
        ...
        Node arrin[] = new Node[100005];//空间换时间,地址是5位正数，直接开辟一个100000大小的数组，用和地址数字相同的下标记录
        while (n-- != 0) {
            String s2[] = in.readLine().split(" ");
            int adds = Integer.parseInt(s2[0]);
            int data = Integer.parseInt(s2[1]);
            int next = Integer.parseInt(s2[2]);
            arrin[adds] = new Node(adds, data, next);
//            arrin[adds].adds = adds;
//            arrin[adds].data = data;
//            arrin[adds].next = next;
        }
        Node temp = new Node(start, arrin[start].data, arrin[start].next);//优化,将数组里的节点按照地址首尾连接添加到ArrayList中
//        temp.adds = start;//优化
...
//        temp.next = arrin[start].next;
        ...
        arrout.add(new Node(temp.adds, temp.data, temp.next));//优化,添加最后一个节点
